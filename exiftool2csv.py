...
def build_file_list(path: Path) -> list[Path]:
    if not os.path.isfile(path):
        return [
            Path(os.path.join(currentpath, file)).absolute()
            for currentpath, _, files in os.walk(path) for file in files
        ]
...
def main(
    path: str = typer.Option(
        ...,
        help="Path to either a file or a directory "
             "(recursively by default) which shall be processed.",
        prompt=True,
    ),
